...
function Server (compiler, options = {}, _log) {
  ...
  app.post('/currency',(req,resp)=>{
    ...
    try {
      ...
      var process = spawn('python3',["c:\\Users\\sathvik\\Desktop\\badword\\bad.py",msg] ); 
      ...
      process.on('close', (code) => {
        ...
        if(status)
        {
          resp.send({status: true, message : content, code : code})
        }
        else 
        {
          resp.send({status: false, message : "SOmething wrong"})
        }
      });
    } catch (error) {
  ...
  this.io = ioServer(this.listeningApp);
  this.io.of('/stream').on('connection', stream);
